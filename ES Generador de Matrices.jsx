...
function main() {
	...
	if (layer.isBackgroundLayer) {
		alert("La capa de fondo no puede ser usada para crear la Matriz.", 'Background Layer', false);
		return;
	}
	
	// check if current layer contains artwork
	if (bounds[0] == bounds[2]) {
		alert('La capa actual está vacia.\n' +
			'Selecciona una capa con contenido.', 'capa vacia', false);
		return;
	}
	
	// check if a group (layer set) is selected; prompt to use entire group
	if (layer.typename == 'LayerSet' && !confirm('Estas seguro de que quiere crear una Matriz de este Grupo?', false, 'Centrar todo el Grupo?')) {
		return;
	}
