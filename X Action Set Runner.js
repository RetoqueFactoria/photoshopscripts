...
GenericUI = function() {
  ...
  self.documentation = "Script de Photohop";
  ...
  self.notesTxt   = 'COMO FUNCIONA?:';
  self.processTxt = 'Procesar';
  self.cancelTxt  = 'Cancelar';
...
RunnerUI = function() {
  ...
  self.title = 'Procesador de Acciones';
  self.notesSize = 130;
  self.winRect = {
    x: 200,
    y: 200,
  ...
  self.documentation =
    "Seleccione un Grupo de Acciones a la izquierda. Seleccione las Acciones de la derecha con Ctrl y Shift.";
...
RunnerUI.prototype.createPanel = function(pnl, ini, doc) {
  ...
  if (sets.length == 0) {
    alert("No hay Grupos de Acciones disponibles");
    return;
  }

  pnl.add('statictext', [xx, yy, xx+200, yy+20], 'Grupo de Acciones');
  xx += 230;
  pnl.add('statictext', [xx, yy, xx+250, yy+20], 'Selecciona Acciones');
  xx = xOfs;
  yy += 30;
  ...
  pnl.add('statictext', [xx, yy, xx+140, yy+20], 'Procesar Acciones:');
  ...
  pnl.add('statictext', [xx,yy,xx+90,yy+20], 'Exportar:');
  xx += 100;
...
RunnerUI.outfBrowse = function() {
  ...
  var folder = Stdlib.selectFolder("Seleccione Carpeta de Exportacion", def);
  if (folder) {
...
RunnerUI.prototype.validatePanel = function(pnl) {
  ...
  try {
    ...
    if (opts.actions.length == 0) {
      return self.errorPrompt("No se han Seleccionado Acciones");
    }
    ...
    if (pnl.outf.text) {
      ...
      if (!f.exists && !f.create()) {
        return self.errorPrompt("No se pudo Crear o Encontrar Carpeta '" + f + "'"
                                + f.error);
      }
    } else {
      if (!opts.sequential && opts.actions.length > 1) {
        return self.errorPrompt("Especifque la Carpeta de Exportacion.");
      }
    }
...
function main() {
  if (app.documents.length == 0) {
    alert("Abre un Documento antes de iniciar el Procesador de Acciones.");
    return;
  }
