...
try{
    if (activeDocument.activeLayer.kind != undefined && activeDocument.activeLayer.isBackgroundLayer == false && emptyLayer == false){
        ...
        alert( "Seleccione una capa con contenido.");
    }
}catch(err){
    alert(err)
}
...
function createDialog(){
    ...
    var dlg = new Window('dialog', 'Dividir en Capas');
    dlg.alignChildren ='left';

    dlg.gap = dlg.add('group')
    dlg.gap.orientation= 'row';
    dlg.gap.txt=dlg.gap.add('statictext', undefined,'Dividir cuando el espacio es más largo que');
    dlg.gap.input=dlg.gap.add('edittext', undefined,tolerance);
        dlg.gap.input.preferredSize = [20,20];
    dlg.gap.txt2=dlg.gap.add('statictext', undefined,'px');
    dlg.gap.btnPreview= dlg.gap.add('button', undefined,'Previsualizar');
        dlg.gap.btnPreview.preferredSize = [55,20]
    
    dlg.naming = dlg.add('panel',undefined,'Renombrar Capas') 
    dlg.naming.alignChildren ='left';
        dlg.naming.suffix = dlg.naming.add('group')
        dlg.naming.suffix.orientation= 'row';
        dlg.naming.suffix.txt=dlg.naming.suffix.add('statictext', undefined,'Sufijo:');
        dlg.naming.suffix.input=dlg.naming.suffix.add('edittext', undefined,suffix);
            dlg.naming.suffix.input.preferredSize = [60,20];

        dlg.naming.suffix.chkbox = dlg.naming.suffix.add('checkbox', undefined, 'Añadir Numeral')
    ...
    dlg.btnPnl.cancelBtn = dlg.btnPnl.add('button', undefined, 'Cancelar', {name:'cancel'});
